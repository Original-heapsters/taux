...
export class MusicController {
	...
	@Get('/playlist/:id')
	async getPlaylist(@Param('id') playlistId: string): Promise<{}> {
	...
	@Get('/track/:id')
	async getTrack(@Param('id') trackId: string): Promise<{}> {
		return await this.spotifyService.getTrack(trackId);
	}

	@Get('/track/:id/features')
	async getTrackFeatures(@Param('id') trackId: string): Promise<{}> {
